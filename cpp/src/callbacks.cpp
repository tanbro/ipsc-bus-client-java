...
static inline JNIEnv* get_current_thread_env() {
  ...
  switch (err) {
    case JNI_EDETACHED:
      ...
      if (err != JNI_OK) {
        snprintf(err_buf, 64, "AttachCurrentThread failed, JNI error code %d",
                 err);
...
static inline void trace_log(const char* msg, bool is_err) {
  ...
  env->CallStaticVoidMethod(cls_client, meth_client_log, jbytes_msg,
                            (jboolean)is_err);
...
void connection(void* arg, unsigned char local_client_id,
  ...
  char buf[64] = {0};
  snprintf(buf, 64, "<thread %lu> >>> connection(%x, %d, %d)", self_thread,
           local_client_id, accesspoint_unit_id, ack);
  trace(buf);
  JNIEnv* env = get_current_thread_env();
  env->CallStaticVoidMethod(cls_client, meth_client_connect,
                            (jbyte)local_client_id, (jint)accesspoint_unit_id,
                            (jint)ack);
  memset(buf, 0, 64);
  snprintf(buf, 64, "<thread %lu> <<< connection(%x, %d, %d)", self_thread,
           local_client_id, accesspoint_unit_id, ack);
...
void disconnect(void* arg, unsigned char local_client_id) {
  pthread_t self_thread = pthread_self();
  char buf[64] = {0};
  snprintf(buf, 64, "<thread %lu> >>> disconnect(%p, %x)", self_thread, arg,
           local_client_id);
  trace(buf);
  JNIEnv* env = get_current_thread_env();
  env->CallStaticVoidMethod(cls_client, meth_client_disconnect,
...
void recvdata(void* arg, unsigned char local_client_id,
  ...
  env->CallStaticVoidMethod(
      cls_client, meth_client_recvdata, (jbyte)head->cmd, (jbyte)head->cmdtype,
      (jbyte)head->src_unit_id, (jbyte)head->src_unit_client_id,
      (jbyte)head->src_unit_client_type, (jbyte)head->dest_unit_id,
      (jbyte)head->dest_unit_client_id, (jbyte)head->dest_unit_client_type,
      jbytes_data);
...
void global_connect(void* arg, char unit_id, char client_id, char client_type,
  ...
  pthread_t self_thread = pthread_self();
  char buf_log[128] = {0};
  snprintf(buf_log, 128,
           "<thread %lu> >>> global_connect(%p, %x, %x, %x, %x, %x, %s)",
           self_thread, arg, unit_id, client_id, client_type, accesspoint_unit,
           status, add_info);
  trace(buf_log);
  if (!add_info) return;
  if (!add_info[0]) return;
  ...
  env->CallStaticVoidMethod(cls_client, meth_client_globalconenct,
                            (jbyte)unit_id, (jbyte)client_id,
                            (jbyte)client_type, (jbyte)status, txt);
...
void trace(const char* msg) { trace_log(msg, false); }

void trace_err(const char* msg) { trace_log(msg, true); }
