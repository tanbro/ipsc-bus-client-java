...
jint JNICALL Java_com_hesong_ipsc_busnetcli_Client_initiateLibrary(JNIEnv * env, jclass cls, jbyte unit_id) {
  ...
  cls_client = cls;
  // Init
  // int result = SmartBusNetCli_InitEx((unsigned char) unit_id, "OpenSignalProcess=1");
...
void JNICALL Java_com_hesong_ipsc_busnetcli_Client_releaseLibrary(JNIEnv * env, jclass cls) {
  cls_client = NULL;
...
jint JNICALL Java_com_hesong_ipsc_busnetcli_Client_createConnect(
  ...
  if (!isCopy)
    throw runtime_error("JNI GetStringUTFChars copy is not made");
  const char *pc_author_username = env->GetStringUTFChars(author_username, &isCopy);
  if (!isCopy)
    throw runtime_error("JNI GetStringUTFChars copy is not made");
...
jint JNICALL Java_com_hesong_ipsc_busnetcli_Client_sendData(JNIEnv * env,
  ...
  if (!isCopy)
    throw runtime_error("JNI GetByteArrayElements copy is not made");
